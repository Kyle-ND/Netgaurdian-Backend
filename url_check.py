    ...
    try:
        ...
        if stats.get("malicious", 0) > 0 or stats.get("suspicious", 0) > 0:
            ...
            try:
                ...
                description = f"You had a phishing url in your email"
                date = str(datetime.datetime.now())
                log_incident(user_id, "email phishing", description, "gmail", 5, date)
                print("logged:", url)
            except Exception as e:
                print(f"Failed to log incident for gmail: {str(e)}")
        else:
            result = "safe"
